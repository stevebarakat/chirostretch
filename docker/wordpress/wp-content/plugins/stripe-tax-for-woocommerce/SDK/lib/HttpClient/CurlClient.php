...
use Stripe\StripeTaxForWooCommerce\SDK\lib\Exception;
use Stripe\StripeTaxForWooCommerce\SDK\lib\Stripe;
use Stripe\StripeTaxForWooCommerce\SDK\lib\Util;
...
class CurlClient implements ClientInterface, StreamingClientInterface {
	...
	/** @var \Stripe\StripeTaxForWooCommerce\SDK\lib\Util\RandomGenerator */
	 ...
	 * @param null|\Stripe\StripeTaxForWooCommerce\SDK\lib\Util\RandomGenerator $randomGenerator
	 */
	 ...
	 *   <li>\Stripe\StripeTaxForWooCommerce\SDK\lib\Util\CaseInsensitiveArray $rheaders The response headers</li>
	...
	private function constructRequest( $method, $absUrl, $headers, $params, $hasFile ) {
		...
		// force IPv4 resolving as Stripe\StripeTaxForWooCommerce\SDK\lib's API servers are only accessible over
	...
	private function useHeadersToDetermineWriteCallback( $opts, $determineWriteCallback ) {
		...
		$writeCallbackWrapper = function ( $curl, $data ) use ( &$writeCallback, &$rheaders, &$determineWriteCallback ) {
			if ( null === $writeCallback ) {
				$writeCallback = \call_user_func_array( $determineWriteCallback, array( $rheaders ) );
			}

			return \call_user_func_array( $writeCallback, array( $curl, $data ) );
		};

		return array( $headerCallback, $writeCallbackWrapper );
	}
	...
	public function executeStreamingRequestWithRetries( $opts, $absUrl, $readBodyChunk ) {
		...
		$determineWriteCallback = function ( $rheaders ) use (
			...
			if ( $rcode < 300 ) {
				...
				return function ( $curl, $data ) use ( &$readBodyChunk ) {
					...
					\call_user_func_array( $readBodyChunk, array( $data ) );
		...
		while ( true ) {
			...
			if ( \is_callable( $this->getRequestStatusCallback() ) ) {
				\call_user_func_array(
					...
					array( $rbody, $rcode, $lastRHeaders, $errno, $message, $shouldRetry, $numRetries )
				);
			}
		...
		return array( $rbody, $rcode, $lastRHeaders );
	}
	...
	private function handleCurlError( $url, $errno, $message, $numRetries ) {
		switch ( $errno ) {
			...
			case \CURLE_OPERATION_TIMEOUTED:
				$msg = "Could not connect to Stripe\StripeTaxForWooCommerce\SDK\lib ({$url}).  Please check your "
				. 'internet connection and try again.  If this problem persists, '
				. "you should check Stripe\StripeTaxForWooCommerce\SDK\lib's service status at "
			...
			case \CURLE_SSL_PEER_CERTIFICATE:
				$msg = "Could not verify Stripe\StripeTaxForWooCommerce\SDK\lib's SSL certificate.  Please make sure "
			...
			default:
				$msg = 'Unexpected error communicating with Stripe\StripeTaxForWooCommerce\SDK\lib.  '
				. 'If this problem persists,';
		}
	 ...
	 * @param int                                                                     $errno
	 * @param int                                                                     $rcode
	 * @param array|\Stripe\StripeTaxForWooCommerce\SDK\lib\Util\CaseInsensitiveArray $rheaders
	 * @param int                                                                     $numRetries
	 *
	 * @return bool
	 */
	...
	private function canSafelyUseHttp2() {
		...
		// (cf. https://github.com/curl/curl/issues/2416), which Stripe\StripeTaxForWooCommerce\SDK\lib use.
