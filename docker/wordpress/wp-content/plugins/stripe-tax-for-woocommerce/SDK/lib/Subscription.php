 ...
 * @property null|string|\Stripe\StripeTaxForWooCommerce\SDK\lib\StripeObject $application ID of the Connect Application that created the subscription.
 * @property null|float $application_fee_percent A non-negative decimal between 0 and 100, with at most two decimal places. This represents the percentage of the subscription invoice total that will be transferred to the application owner's Stripe\StripeTaxForWooCommerce\SDK\lib account.
 ...
 * @property null|\Stripe\StripeTaxForWooCommerce\SDK\lib\StripeObject $billing_thresholds Define thresholds at which an invoice will be sent, and the subscription advanced to a new billing period
 ...
 * @property null|\Stripe\StripeTaxForWooCommerce\SDK\lib\StripeObject $cancellation_details Details about why this subscription was cancelled
 * @property string $collection_method Either <code>charge_automatically</code>, or <code>send_invoice</code>. When charging automatically, Stripe\StripeTaxForWooCommerce\SDK\lib will attempt to pay this subscription at the end of the cycle using the default source attached to the customer. When sending an invoice, Stripe\StripeTaxForWooCommerce\SDK\lib will email your customer an invoice with payment instructions and mark the subscription as <code>active</code>.
 ...
 * @property string|\Stripe\StripeTaxForWooCommerce\SDK\lib\Customer $customer ID of the customer who owns the subscription.
 ...
 * @property null|string|\Stripe\StripeTaxForWooCommerce\SDK\lib\Account|\Stripe\StripeTaxForWooCommerce\SDK\lib\BankAccount|\Stripe\StripeTaxForWooCommerce\SDK\lib\Card|\Stripe\StripeTaxForWooCommerce\SDK\lib\Source $default_source ID of the default payment source for the subscription. It must belong to the customer associated with the subscription and be in a chargeable state. If <code>default_payment_method</code> is also set, <code>default_payment_method</code> will take precedence. If neither are set, invoices will use the customer's <a href="https://stripe.com/docs/api/customers/object#customer_object-invoice_settings-default_payment_method">invoice_settings.default_payment_method</a> or <a href="https://stripe.com/docs/api/customers/object#customer_object-default_source">default_source</a>.
 * @property null|\Stripe\StripeTaxForWooCommerce\SDK\lib\TaxRate[] $default_tax_rates The tax rates that will apply to any subscription item that does not have <code>tax_rates</code> set. Invoices created will have their <code>default_tax_rates</code> populated from the subscription.
 * @property null|string $description The subscription's description, meant to be displayable to the customer. Use this field to optionally store an explanation of the subscription for rendering in Stripe\StripeTaxForWooCommerce\SDK\lib surfaces and certain local payment methods UIs.
 * @property null|\Stripe\StripeTaxForWooCommerce\SDK\lib\Discount $discount Describes the current discount applied to this subscription, if there is one. When billing, a discount applied to a subscription overrides a discount applied on a customer-wide basis.
 * @property null|int $ended_at If the subscription has ended, the date the subscription ended.
 * @property \Stripe\StripeTaxForWooCommerce\SDK\lib\Collection<\Stripe\StripeTaxForWooCommerce\SDK\lib\SubscriptionItem> $items List of subscription items, each with an attached price.
 * @property null|string|\Stripe\StripeTaxForWooCommerce\SDK\lib\Invoice $latest_invoice The most recent invoice this subscription has generated.
 ...
 * @property null|string|\Stripe\StripeTaxForWooCommerce\SDK\lib\Account $on_behalf_of The account (if any) the charge was made on behalf of for charges associated with this subscription. See the Connect documentation for details.
 * @property null|\Stripe\StripeTaxForWooCommerce\SDK\lib\StripeObject $pause_collection If specified, payment collection for this subscription will be paused.
 * @property null|\Stripe\StripeTaxForWooCommerce\SDK\lib\StripeObject $payment_settings Payment settings passed on to invoices created by the subscription.
 * @property null|\Stripe\StripeTaxForWooCommerce\SDK\lib\StripeObject $pending_invoice_item_interval Specifies an interval for how often to bill for any pending invoice items. It is analogous to calling <a href="https://stripe.com/docs/api#create_invoice">Create an invoice</a> for the given subscription at the specified interval.
 * @property null|string|\Stripe\StripeTaxForWooCommerce\SDK\lib\SetupIntent $pending_setup_intent You can use this <a href="https://stripe.com/docs/api/setup_intents">SetupIntent</a> to collect user authentication when creating a subscription without immediate payment or updating a subscription's payment method, allowing you to optimize for off-session payments. Learn more in the <a href="https://stripe.com/docs/billing/migration/strong-customer-authentication#scenario-2">SCA Migration Guide</a>.
 * @property null|\Stripe\StripeTaxForWooCommerce\SDK\lib\StripeObject $pending_update If specified, <a href="https://stripe.com/docs/billing/subscriptions/pending-updates">pending updates</a> that will be applied to the subscription once the <code>latest_invoice</code> has been paid.
 * @property null|string|\Stripe\StripeTaxForWooCommerce\SDK\lib\SubscriptionSchedule $schedule The schedule attached to the subscription
...
class Subscription extends ApiResource {
	 ...
	 *
	 * @return \Stripe\StripeTaxForWooCommerce\SDK\lib\Subscription the updated subscription
	 ...
	 * @param null|array|string $opts
	 *
	 * @return \Stripe\StripeTaxForWooCommerce\SDK\lib\Subscription the canceled subscription
	 * @throws \Stripe\StripeTaxForWooCommerce\SDK\lib\Exception\ApiErrorException if the request fails
	 */
	 ...
	 * @param null|array|string $opts
	 *
	 * @return \Stripe\StripeTaxForWooCommerce\SDK\lib\Subscription the resumed subscription
	 * @throws \Stripe\StripeTaxForWooCommerce\SDK\lib\Exception\ApiErrorException if the request fails
	 */
	 ...
	 * @param null|array|string $opts
	 *
	 * @return \Stripe\StripeTaxForWooCommerce\SDK\lib\SearchResult<\Stripe\StripeTaxForWooCommerce\SDK\lib\Subscription> the subscription search results
	 * @throws \Stripe\StripeTaxForWooCommerce\SDK\lib\Exception\ApiErrorException if the request fails
	 */
