...
"[project]/src/app/api/bookings/add-to-cart/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function POST(request) {
    try {
        ...
        const cookieHeader = cookieStore.getAll().filter((c)=>c.name.startsWith("wp_woocommerce_session") || c.name.startsWith("woocommerce_")).map((cookie)=>`${cookie.name}=${cookie.value}`).join("; ");
        const res = await fetch(`${WP_URL}/wp-json/chirostretch/v1/bookings/add-to-cart`, {
            method: "POST",
            headers: {
        ...
        if (setCookie) {
            const cookies = setCookie.split(/,(?=\s*(?:woocommerce_|wp_woocommerce_))/);
...
//# sourceMappingURL=%5Broot-of-the-server%5D__eeed7ab2._.js.map